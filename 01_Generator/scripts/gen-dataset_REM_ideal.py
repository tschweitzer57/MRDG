...
def gen_detection_mapping(robots):
    rid_list = robots.copy()
    np.random.shuffle(rid_list)
    com_map = []
    
    for i in range(len(rid_list)//2):
        com_map.append((rid_list[2*i], rid_list[2*i+1]))
...
def generate_dataset(config_file_path, output_dir):
    ...
    if builder.config.landmarks is not None:
        # Generate landmarks
        builder.gen_lk_amers(builder.config.landmarks['number'])
    ...
    for pose_num in range(1,builder.config.dataset_opts['number_poses']):
        ...
        # Add inter direct loop closures
        if builder.config.lc_inter_direct is not None:

            # Add range measurement
            if builder.config.lc_inter_direct.get('range') is not None:
                freq = builder.config.lc_inter_direct['range']['frequency']
                if pose_num % freq == 0:
                    builder.incr_stamp()
                    com_map = gen_detection_mapping(builder.robots)
                    for ra, rb in com_map:
                        builder.add_lc_inter_direct('range', pose_num, ra, rb, modality='duplex')
            
            # Add pose measurement
            if builder.config.lc_inter_direct.get('pose') is not None:
                freq = builder.config.lc_inter_direct['pose']['frequency']
                if pose_num % freq == 0:
                    builder.incr_stamp()
                    com_map = gen_detection_mapping(builder.robots)
                    for ra, rb in com_map:
...
if __name__ == "__main__":
    ...
    config_file = './configs/REM_1/mesa_ideal.json'
    output_dir = './saved_outputs/REM_1'

    generate_dataset(config_file, output_dir)
